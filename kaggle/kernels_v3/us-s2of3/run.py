...
#
# This runner clones the repo at RUNTIME, so a kernel launched now executes
# whatever is on main when it starts -- not what was on main when the model it
# loads was trained. That bit us: competition features (20 -> 32) were pushed
# while v2 shards were queued, and the France shard spent 113 minutes blocking
...
# them together.
COMMIT = "c6348a4"              # "" = latest main (only safe when nothing is in flight)
...
if not SRC.exists():
    if COMMIT:
        # Full clone: --depth 1 cannot check out an arbitrary older commit.
        sh(f"git clone {REPO} {CHECKOUT}")
        sh(f"cd {CHECKOUT} && git checkout --quiet {COMMIT}")
    else:
        sh(f"git clone --depth 1 {REPO} {CHECKOUT}")
assert SRC.exists(), f"src not found at {SRC}"
sh(f"cd {CHECKOUT} && git log -1 --oneline", check=False)   # record what ran
if COMMIT:
    actual = subprocess.run(f"cd {CHECKOUT} && git rev-parse HEAD", shell=True,
                            capture_output=True, text=True).stdout.strip()
    assert actual.startswith(COMMIT), f"wanted {COMMIT}, got {actual}"
    print(f"PINNED to {COMMIT}", flush=True)
...
if STAGE == "train":
    ...
    cmd = [sys.executable, "-m", "pipeline", "predict",
           ...
           "--country", STAGE, "--shard", "2", "--shards", "3",
           "--output-dir", str(OUTPUT)]
